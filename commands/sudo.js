    ...
    try {
        ...
        if (action === "add") {
            if (!list.includes(participant)) {
                ...
                await client.sendMessage(remoteJid, { text: `⚠️ Cette utilisateur est déjà sur le trône` })
            }
            ...
            if (index !== -1) {
                ...
                await client.sendMessage(remoteJid, { text: `⚠️ Cette utilisateur n'est pas sur sur le trône` })
            }
        }
    } catch (error) {
